...
```text
app/
├── page.tsx              # メイン UI（入力・今回の生成結果・生成用入力表示・編集モーダル）
├── components/
...
| `npm run lint` | ESLint |
| `npm run test:e2e` | Playwright 全 28 ケース（`next dev` を Playwright 設定で起動。`.env.local` 読み込み）。合計所要の目安 **約 30～55s**（直近フル run 記録は **約 49s**。[e2e-test-run-summary.md](docs/e2e/e2e-test-run-summary.md) 参照） |
...
| E2E（Playwright） | `npm run test:e2e` 全 28 ケース想定（合計 **約 30～55s** の目安、直近記録 **約 49s**。負荷・API 応答で変動）。前提は `e2e/README.md` |
