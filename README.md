...
| `npm run lint` | ESLint |
| `npm run test:e2e` | Playwright 全 28 ケース（`next dev` を Playwright 設定で起動。`.env.local` 読み込み）。合計所要の目安 **約 30～55s**（[e2e-test-run-summary.md](docs/e2e/e2e-test-run-summary.md) の直近 run） |
...
| E2E（Playwright） | `npm run test:e2e` 全 28 ケース想定（合計 **約 30～55s** の目安。負荷・API 応答で変動）。前提は `e2e/README.md` |
